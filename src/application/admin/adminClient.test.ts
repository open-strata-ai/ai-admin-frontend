...
describe('adminClient', () => {
  ...
  it('listTenants() GETs /v1/admin/tenants', async () => {
    ...
    expect(url.endsWith('/v1/admin/tenants')).toBe(true);
  ...
  it('getGovernance() GETs /v1/admin/global-resources', async () => {
    ...
    expect(url.endsWith('/v1/admin/global-resources')).toBe(true);
  });
  ...
  it('getGpuPool() GETs /v1/admin/gpu-pool', async () => {
    ...
    const [url] = (global.fetch as any).mock.calls[0];
    expect(url.endsWith('/v1/admin/gpu-pool')).toBe(true);
  });

  it('getAudit() GETs /v1/admin/audit with query params', async () => {
    ...
    const [url] = (global.fetch as any).mock.calls[0];
    expect(url.endsWith('/v1/admin/audit?scope=tenant&tenantId=local')).toBe(true);
  });
});
